...
class NoteRepository @Inject constructor(
    ...
    private val _dataLiveData = MutableLiveData<List<AllDataModel>>()
    val dataLiveData: LiveData<List<AllDataModel>> = _dataLiveData
    val allNotesLiveData: LiveData<List<AllDataModel>> = localDataSource.allNotesLiveData
    ...
    suspend fun fetchData(): List<AllDataModel> {
        ...
        if (context.isNetworkAvailable()) {
            ...
            Toast.makeText(context, "Internet is There not there  ", Toast.LENGTH_SHORT).show()
        }
